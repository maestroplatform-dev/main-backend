...
exports.sendTeacherCredentialsEmail = sendTeacherCredentialsEmail;
 ...
 */
async function sendTeacherCredentialsEmail(email, password, teacherName = 'Teacher') {
 ...
 */
async function sendViaResend(email, subject, htmlContent) {
    if (!resend) {
 ...
 */
async function queueEmail(toEmail, subject, htmlContent, templateData) {
    try {
        await database_1.default.email_queue.create({
            data: {
                to_email: toEmail,
                subject,
...
async function processEmailQueue() {
    ...
    const pendingEmails = await database_1.default.email_queue.findMany({
        where: {
            status: 'pending',
    ...
    for (const queuedEmail of pendingEmails) {
        try {
            ...
            // Mark as sent
            await database_1.default.email_queue.update({
 ...
 */
function generateCredentialsEmailHTML(email, password, name) {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
