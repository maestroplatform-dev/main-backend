...
export class OTPService {
   ...
   */
  static async generateAndSendOTP(email: string): Promise<{ code: string }> {
    ...
    logger.info({ email, code }, '📧 OTP generated')
  ...
  static async verifyOTP(email: string, code: string): Promise<boolean> {
    ...
    if (otpRecord.attempts >= 5) {
      otpStore.delete(email)
      throw new AppError(429, 'Too many failed attempts', 'OTP_MAX_ATTEMPTS')
    }
  ...
  static cleanupExpiredOTPs(): void {
    ...
    if (cleaned > 0) {
      logger.info({ count: cleaned }, '🧹 Cleaned up expired OTPs')
    }
  }
   ...
   */
  static async resendOTP(email: string): Promise<{ code: string }> {
